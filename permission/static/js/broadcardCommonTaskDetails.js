    ...
    var privateHostQuickImport = {
        ...
        tmpEdit: function() {
            ...
            $('.save').on('click', function(event) {
                ...
                var xuliehao = new Array();
                var suoshushebei = new Array();
                var xinghao = new Array();
                var caowei = new Array();
                var duankoushu = new Array();
                ...
                $(".tmp-row .序列号").each(function(i){
                    if($(this).hasClass('序列号')){
                        if($(this).children('input').val().trim() == '') {
                            $(this).children('input').focus();
                            $(this).children('input').tooltip('show');
                            flag=false;
                            ...
                            haveHostFlag = true;
                            xuliehao.push($(this).children('input').val().trim());
                        }
                    }
                });
                if(!flag){
                ...
                $(".tmp-row .所属设备").each(function(i){
                    if($(this).hasClass('所属设备')){
                        suoshushebei.push($(this).children('input').val().trim())
                    }
                });

                $(".tmp-row .型号").each(function(i){
                    if($(this).hasClass('型号')){
                        xinghao.push($(this).children('input').val().trim())
                    }
                });

                $(".tmp-row .槽位").each(function(i){
                    if($(this).hasClass('槽位')){
                        caowei.push($(this).children('input').val().trim())
                    }
                });

                $(".tmp-row .端口数").each(function(i){
                    if($(this).hasClass('端口数')){
                        duankoushu.push($(this).children('input').val().trim())
                    }
                });

                for (var k = 0, length = zichanbianhao.length; k < length; k++) {
                    var tmp = {};
                    tmp['资产编号']=zichanbianhao[k];
                    tmp['序列号']=xuliehao[k];
                    tmp['所属设备'] = suoshushebei[k];
                    tmp['型号'] = xinghao[k];
                    tmp['槽位'] = caowei[k];
                    tmp['端口数'] = duankoushu[k];
                    param.push(tmp);
                }
                ...
                if(flag){
                    $.ajax({
                        ...
                        data:{"req":JSON.stringify(param),"nodetype":"net_board_card"},
                        method:'post',
                        dataType:'json',
