$(document).ready(function() {
    ...
    */
    var privateKendoGridObj = {
        columns:[
            ...
            {field:"型号",title:'型号',filterable:true},
            {field:"槽位",title:'槽位',filterable:true},
            {field:"端口数",title:'端口数',filterable:true}
        ],
        dataSource:{
            transport: {
                read: {
    ...
    */
    $('.k-grid-quickDistribute').on('click', function(e){
        ...
        param['nodetype'] = 'net_board_card';

        var options = {
            title:'确认',
            width:310,
            content: '确认要将已选择板卡导入CMDB ?',
            okValue:"继续",
            cancelValue:"我再想想",
            ok:function (){
                var d = dialog({
                    ...
                    success:function(response){
                        ...
                        if(response.success){
                            var distributeHostDialog = dialog({
                                ...
                                okValue:"确定",
                                ok:function (){
                                    location.href = '/page/device/broadcardimport';
                                }
                            });
