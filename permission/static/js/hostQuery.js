...
!function(){
    ...
    */
    var treeContainerChartObj = {
        ...
        select:function(e){
            ...
            gridObj.dataSource.transport.read.url = '/api/host/gethosts';
            //gridObj.dataSource.transport.read.data = {"z":"D","z":"L"};
            //gridObj.dataSource.transport.read.url = '/api/host/search';
    ...
    */
    $('#filter_module_mine').click(function(e){
        ...
        //现改为搜索维护人为当前用户的主机
        gridObj.dataSource.transport.read.data = {"query":"m:"+$.cookie("username")};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();
    ...
    */
    $('#filter_module_all').click(function(e){
        ...
        //现改为搜索当前业务所有主机
        gridObj.dataSource.transport.read.data = {"query":"i:"};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    /**
    * 查询条件按钮"盘点有效"点击事件处理函数
    */
    $('#filter_module_pandian').click(function(e){
        e.stopPropagation();

        gridObj.dataSource.transport.read.data = {"query":"z:D,z:L"};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    * 查询条件按钮"酒仙桥"点击事件处理函数
    */
    $('#filter_module_jxq').click(function(e){
        e.stopPropagation();

        gridObj.dataSource.transport.read.data = {"query":"j:酒仙桥"};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    * 查询条件按钮"aws"点击事件处理函数
    */
    $('#filter_module_aws').click(function(e){
        e.stopPropagation();

        gridObj.dataSource.transport.read.data = {"query":"j:aws"};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    /**
    * 查询条件按钮"Dell"点击事件处理函数
    */
    $('#filter_module_dell').click(function(e){
        e.stopPropagation();

        gridObj.dataSource.transport.read.data = {"query":"z:D"};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    * 查询条件按钮"Ibm"点击事件处理函数
    */
    $('#filter_module_ibm').click(function(e){
        e.stopPropagation();

        gridObj.dataSource.transport.read.data = {"query":"z:L"};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    /**
    * 查询条件按钮"qita"点击事件处理函数
    */
    $('#filter_module_qita').click(function(e){
        e.stopPropagation();

        gridObj.dataSource.transport.read.data = {"query":"z:T"};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    /**
    * 查询条件按钮"刷新cache"点击事件处理函数
    */
    $('#filter_module_flush').click(function(e){
        e.stopPropagation();
        $.ajax({
            url:'/api/host/flushcache',
            dataType:'json',
            method:'get',
    ...
    */
    ...
    //资源池按钮
    $('#pool_button').on('click',function(){
        gridObj.dataSource.transport.read.data = {"tagid":207};
        gridObj.dataSource.transport.read.url = '/api/host/gethosts';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    */
    CC.host.topology={
        ...
        init:function(data){
            ...
            var setting = { 
                ...
                callback: {
                    onClick: function (event,treeId,treeNode) {
                        gridObj.dataSource.transport.read.data = {"tagid":treeNode.id};
                        gridObj.dataSource.transport.read.url = '/api/host/gethosts';
                        CC.host.hostlist.init();
                        $('#batRes').hide();
                        $('#batDel').show();
            ...
            var g_tag_obj = {"id":0,"name":"","parentid":0,"fullname":"","fulltag":"","ctime":"0001-01-01T00:00:00Z","utime":"0001-01-01T00:00:00Z"};

            $.fn.zTree.init($("#tagtree"), setting, data);
            var treeObj = $.fn.zTree.getZTreeObj("tagtree");
            if(g_tag_obj) {
                var objTag = treeObj.getNodeByParam("id", g_tag_obj.parentid, null);
                treeObj.expandNode(objTag,true,false,false);
            }
        },
    ...
    */
    CC.host.hostlist={
        ...
        init:function(){
            ...
            me._init_checkAll();
            me._init_copy('.copy-inner-ip','ipaddress');
            me._init_copy('.copy-domain','主机名');
            me._init_copy('.copy-asset-id','资产编号');
        ...
        events:function(){
            ...
            hostlist.element.on('click.kendoGrid', '.a-innerip', function(e){
                ...
                var param = {};
                param['hostid'] = data.id;
                $.ajax({
                    url:'/page/host/details',
                    data:param,
                    dataType:'html',
                    method:'post',
            ...
            hostlist.element.on('click.kendoGrid', '#batRes', function (e){
                ...
                var gridBatRes = dialog({
                    title:'确认',
                    width:300,
                    content: '确认是否将已勾选的<i class="redFont">'+hostId.length+'</i>台主机上交至<i data-toggle="tooltip" data-placement="bottom" title="同一开发商下不同业务之间共享资源的缓冲池" class="redFont-e">资源池</i>',
                    okValue:"确定",
                    cancelValue:"取消",
                    ok:function (){
                        var d = dialog({
        ...
        _initColumns:function(){
            ...
            if(!me.defaultColumns || me.defaultColumns.length===0){
                ...
                me.defaultColumns = ["操作系统","设备型号","sn","硬盘大小","ipaddress","主机名","物理CPU数","CPU核数","是否虚拟化","内核","内存"]
            }
        ...
        _copyIp:function(btn,clipboard,filedname){
            ...
            if(list.length){
                var result = $.grep(list, function(n) {return $.trim(n).length > 0;});
                clipboard.setData('text/plain',result.join("\n"));
            }else{
                if($('.k-grid-copyIP').attr('disabled')=='disabled'){
                    return false;
                }
        ...
        gridObj={
            change:function(e){
                ...
                if(userSelection!=null){
                    ...
                    var relativeX = (pos.left-290+lens+scrollLeft);
                    var relativeY = (pos.top-290+scrollTop);
            ...
            dataSource: {//数据源配置项           
                ...
                requestStart: function(){
                    kendo.ui.progress($("#host-list").parent(), true);
                },
                requestEnd: function(){
                    kendo.ui.progress($("#host-list").parent(), false);
                },
                schema: {
            ...
            toolbar: [//头部工具栏kendoToolBar,可以参考ui.toolbar的api
                {
                    ...
                    '<li class="copy-inner-ip" data-type="ipaddress">复制IP</li>'+
                    '<li class="copy-domain" data-type="主机名">复制主机名</li>'+
                    '<li class="copy-asset-id" data-type="资产编号">复制资产编号</li>'+
                    '</ul>'+
                    '</div>',
                    name:'copyIP',
    ...
    */
    $('#host_query_submit').click(function(){
        var param = {};
        var InnerIP = $('#ipaddress').val();
        if(InnerIP){
            param['ipaddress'] = InnerIP.replace(/\s+/g, ',').replace(/[\s,]+$/g, '');
        }
        ...
        $.each(customerQueryFields, function(index, v) {
            var stdProperty = $('#' + v).val();
            if(stdProperty) {
                param[v] = stdProperty.replace(/\s+/g, ',').replace(/[\s,]+$/g, '');
            }
        });

        gridObj.dataSource.transport.read.data = param;
        gridObj.dataSource.transport.read.url = '/api/host/gethostsbycondition';
        CC.host.hostlist.init();
    });

    $('#host_speed_search').click(function(){
        var content=$('#host-search-textarea').val();
        /*没有输入任何值*/
        if(!content){
            return true;
        }
        ...
        if (content.indexOf("\n") > 0 || content.indexOf(",") > 0) {
            //content = content.replace(/\s+/g, ',');/*换行转逗号*/           
            content = content.replace(/\n+/g, ',');/*换行转逗号*/           
        }
        /*多值*/

        gridObj.dataSource.transport.read.data = {"query":content};
        gridObj.dataSource.transport.read.url = '/api/host/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();
