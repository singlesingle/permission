...
!function(){
    ...
    */
    $('#filter_module_all').click(function(e){
        ...
        //现改为搜索当前业务所有主机
        gridObj.dataSource.transport.read.data = {"tagid":379,"nodetype":"net_devices"};
        gridObj.dataSource.transport.read.url = '/api/host/gethosts';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    * 查询条件按钮"交换机"点击事件处理函数
    */
    $('#filter_module_switch').click(function(e){
        e.stopPropagation();

        //现改为搜索当前业务所有主机
        gridObj.dataSource.transport.read.data = {"query":"n:交换机"};
        gridObj.dataSource.transport.read.url = '/api/device/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    });
    ...
    * 查询条件按钮"防火墙"点击事件处理函数
    */
    $('#filter_module_firewall').click(function(e){
        e.stopPropagation();

        //现改为搜索当前业务所有主机
        gridObj.dataSource.transport.read.data = {"query":"n:防火墙"};
        gridObj.dataSource.transport.read.url = '/api/device/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    /**
    * 查询条件按钮"负载均衡"点击事件处理函数
    */
    $('#filter_module_balance').click(function(e){
        e.stopPropagation();

        //现改为搜索当前业务所有主机
        gridObj.dataSource.transport.read.data = {"query":"n:负载均衡"};
        gridObj.dataSource.transport.read.url = '/api/device/search';
        CC.host.hostlist.init();
        $('#batRes').hide();
        $('#batDel').show();    
    ...
    */
    CC.host.hostlist={
        ...
        init:function(){
            ...
            me._init_checkAll();
            me._init_copy('.copy-manage-ip','管理IP');
            me._init_copy('.copy-xuliehao','序列号');
            me._init_copy('.copy-zichanbianhao','资产编号');
    ...
    var wH=$('.content-wrapper').height(),
        ...
        gridObj={
            ...
            dataSource: {//数据源配置项           
                transport: {
                    read: {
                        url: "/api/host/gethosts",
                        data:{'tagid':379,"nodetype":"net_devices"},
                        method:'post',
                        dataType:"json"
                    }
                },
            ...
            toolbar: [//头部工具栏kendoToolBar,可以参考ui.toolbar的api
                {
                    ...
                    '<li class="copy-manage-ip" data-type="管理IP">复制管理IP</li>'+
                    '<li class="copy-xuliehao" data-type="序列号">复制序列号</li>'+
                    '<li class="copy-zichanbianhao" data-type="资产编号">复制资产编号</li>'+
                    '</ul>'+
                    '</div>',
                    name:'copyIP',
    ...
    */
    ...
    $('#net_device_speed_search').click(function(){
        var content=$('#net-device-search-textarea').val();
        /*没有输入任何值*/
        if(!content){
            return true;
        }
