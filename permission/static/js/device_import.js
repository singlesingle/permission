$(document).ready(function() {
    ...
    */
    var privateKendoGridObj = {
        columns:[
            ...
            {field:"管理IP",title:'管理IP',filterable:true},
            {field:"资产编号",title:'资产编号',filterable:true},
            {field:"序列号",title:'序列号',filterable:true},
            {field:"系统名称",title:'系统名称',filterable:true}
        ],
        dataSource:{
            transport: {
                read: {
                    url: "/api/device/getimportdevice",
	 ...
	 var broadcardPrivateKendoGridObj = {
        columns:[
            {field:'checkbox',title:'#',menu:false,width:30,template:'<input type="checkbox" #:data.Checked# value="#:data.HostID#" class="c-grid-checkbox"/>'},
            {field:"资产编号",title:'资产编号',filterable:true},
            {field:"序列号",title:'序列号',filterable:true},
            {field:"槽位",title:'槽位',filterable:true},
			{field:"所属设备",title:'所属设备',filterable:true}
        ],
        dataSource:{
            transport: {
                read: {
                    url: "/api/device/getimportdevice",
                    data:{nodetype:"net_board_card"},
                    data:"",
                    //method:'post',
                    method:'get',
                    dataType:"json"
            ...
            serverPaging: false,
            requestStart: function(){
                kendo.ui.progress($("#broadcard_private").parent(), true);
            },
            requestEnd: function(){
                kendo.ui.progress($("#broadcard_private").parent(), false);
            },
            schema: {
                data: function (response) {
                    return response.data;
        ...
        change: function(e){
            $('.k-grid-delete', '#broadcard_private').attr('disabled', false);

            var grid = $('#broadcard_private').data('kendoGrid');
            var selectedRows = grid.select();
            $(selectedRows).closest('tr').find('input').prop('checked', true);
            for(var i=0,len=selectedRows.length; i<len; i++){
                var data = grid.dataItem($(selectedRows[i]).parents('tr'));
            ...
            if(this.selectNum > 0 && $('.cc_switch_btn').attr('data-fp')=='1'){
                $('.k-grid-quickDistribute', '#broadcard_private').attr('disabled', false);
            }else{
                $('.k-grid-quickDistribute', '#broadcard_private').attr('disabled',true);
            }
        },
        toolbar: [//头部工具栏kendoToolBar,可以参考ui.toolbar的api
            {text:'入库',name:'broadcard-quickDistribute',attr:{"href":"javascript:void(0);","disabled":"true"}},
            //{text:'导入主机',template:"<a class=\"k-button\" id=\"importPrivateHostByExcel\" href=\"javascript:void(0);\"><span class=\"\"></span>导入主机</a>"},
            {text:'快速导入',template:"<a class=\"k-button broadcard-importfast-button\"  data-target=\".importfast-template\" href=\"javascript:void(0);\"><span class=\"\"></span>快速导入</a>"},
            {text:'删除',name:'broadcard-delete',attr:{"href":"javascript:void(0);","disabled":"true"}},
            //{text:'搜索',template:"<div class=\"c-import-search\"><input id=\"filter-private\" type=\"text\" class=\"form-control pull-left\" placeholder=\"搜索...\" /><i class=\"glyphicon glyphicon-search\"></i></div>"},
            //{text:'切换',template:"<div class=\"c-import-fpBtn\"><div class=\"cc_switch_btn cc_switch_btn_right\" data-fp=\"1\"><img class=\"switch\" src=\"/static/img/cc_switch_btn.png\"/><div class=\"num\">未分配</div></div></div>"}
        ]
    };
    ...
    $("#broadcard_private").kendoGrid(broadcardPrivateKendoGridObj);
	...
	$('.broadcard-importfast-button').on('click', function(){
        $('.broadcard-importfast-template-modal').modal();
        return false;
    });
    ...
    (function(){
        ...
        /*表格表头添加checkbox*/
        var type = ['private','broadcard_private'];
    ...
    $('.k-grid-quickDistribute').on('click', function(e){
        ...
        var options = {
            ...
            ok:function (){
                ...
                $.ajax({
                    dialog:d,
                    url:'/api/device/quickdistribute',
                    data:param,
                    method:'post',
                    dataType:'json',
    ...
    */
	...
	$('#broadcard_private').on('change', 'input[type=checkbox]', function(e){
        var type = $(e.target).parents('.tab-pane').attr('id');
        var grid = $('#'+type).data('kendoGrid');
        var filter = grid.dataSource.filter();
    ...
    $(".k-grid-delete").on('click', function(e){
        ...
        var options = {
            ...
            ok:function (){
                ...
                $.ajax({
                    dialog:d,
                    url:'/api/device/deleteimportdevice',
                    data:param,
                    method:'post',
                    dataType:'json',
                    success:function(response){
                        ...
                        if(response.success){
                            var distributeHostDialog = dialog({
                                ...
                                okValue:"确定",
                                ok:function (){
                                    location.href = '/page/device/netdeviceimport';
                                }
                            });
